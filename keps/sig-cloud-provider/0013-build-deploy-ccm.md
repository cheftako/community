...
- [Switching To Cloud Provider Repo And Builds](#switching-to-cloud-provider-repo-and-builds)
   ...
   - [Building CCM For Cloud Providers](#building-ccm-for-cloud-providers)
      ...
      - [Design Options](#design-options)
          - [Staging](#staging)
          - [Importing the Cloud Provider Implementation](#importing-the-cloud-provider-implementation)
   ...
   - [Alternatives](#alternatives)
      ...
      - [Build Location Alternatives](#build-location-alternatives)
          - [Build within K8s/Cloud-Provider](#build-within-k8scloud-provider)
          - [Build within K8s/K8s](#build-within-k8sk8s)
...
Cloud provider specific logic will be removed from the K8s/K8s repository and the generic cloud provider code will be restructured 
...
As the Kubernetes ecosystem grows and more cloud providers implement the cloud provider interface, the number of
dependencies in the Kubernetes core code will increase too, increasing bloat and complexity. One improvement that
can be made to decrease these dependencies is removing the cloud provider implementations from core and into their
own cloud provider owned repositories.

Currently there are eight cloud provider implementations. To prevent this number from growing we have locked Kubernetes
Core to the addition of any new dependencies. This means all new cloud providers have to implement all their pieces
outside of the Core. However, the current cloud providers are still used by core Kubernetes components. In order to
remove the dependencies, changes are required in the following areas:
...
decouple cloud provider fixes from open source releases. Moreover, moving the cloud provider specific code out of
Kubernetes Core means that the core binaries do not need to load unnecessary code.

### Goals

- Get to a point where we do not load the cloud interface for any of kubernetes core processes.
- Remove all cloud provider specific code from K8s/K8s.
...
- Minimize the number and scope of controllers running in the cloud controller manager so as to minimize deviation
...
we can push more code into Staging to make the code available to the 
K8s/Cloud-Provider repos. Currently code which needs to be shared between K8s/K8s itself and other projects is
migrated to Staging. This allows the build machinery to detect things like attempts to make the shared code depend on 
code which is not shared (i.e. disallowing code in Staging from depending on code in K8s/K8s but not in Staging) 
...
#### Importing the Cloud Provider Implementation

Currently in K8s/K8s the cloud providers are included by
[providers.go](https://github.com/kubernetes/kubernetes/blob/master/pkg/cloudprovider/providers/providers.go). In the
short term we would leave that file where it is and rewrite the imports to point to the new locations for the cloud
...
Each K8s/Cloud-Provider repo would import just their cloud provider implementation. This would go in a wrapper file (Essentially
a modified copy of cmd/cloud-controller-manager/controller-manager.go).
...
#### Build within K8s/Cloud-Provider
